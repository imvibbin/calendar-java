    ...
    <>
      {hours.map((hour) => (
        <div key={hour} className="row">
          ...
          {/* Loop through days */}
          {[...Array(7)].map((_, index) => {
            const currentDivId = `hour${hour.toString().padStart(2, "0")}-day${index + 1}`;
            const event = eventsData.events.find((event) => event.info === currentDivId);
            ...
            if (event) {
              ...
              const eventCells = Array.from({ length: eventDuration }, (_, i) =>
                `hour${(hour + i).toString().padStart(2, "0")}-day${index + 1}`
              );
              ...
              return (
                ...
                <div
                  ...
                  className={`col border ${
                    addingEvent && selectedCell === currentDivId ? "add-event" : ""
                  }`}
                ...
                >
                  {addingEvent && selectedCell === currentDivId && (
                    ...
                    <div className="event-overlay">
                      ...
                      />
                      <button onClick={() => saveEvent(currentDivId, eventName, eventDuration)}>
                        Save
                      </button>
                    </div>
                  )}
